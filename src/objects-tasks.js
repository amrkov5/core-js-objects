...
function mergeObjects(objects) {
  const res = {};
  objects.forEach((el) => {
    const objEnt = Object.entries(el);
    objEnt.forEach((e) => {
      const key = e[0];
      const value = e[1];
      if (res[key]) {
...
function makeWord(lettersObject) {
  ...
  Object.values(lettersObject).forEach((el, i) => {
    el.forEach((e) => {
      arr[e] = objKeys[i];
    });
  });
...
function group(array, keySelector, valueSelector) {
  ...
  for (let key, i = 0; i < array.length; i += 1) {
    ...
    if (res.has(key)) {
      res.get(key).push(valueSelector(array[i]));
    } else {
      res.set(key, [valueSelector(array[i])]);
    }
  }
  return res;
}
